...
export async function POST(req: Request) {
  ...
  const { environment, token, reference, payload } = parsed.data;
  ...
  try {
    ...
    logger.warn({ err }, 'focus.emit_test.error');
    return NextResponse.json({ ok: false, error: 'Erro ao emitir na Focus.', detail: String(err) }, { status: 502 });
  }
}
